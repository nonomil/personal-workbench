...
- 任务 ID：`T20260815-B4`
- 状态：`in-progress`（C2/C4/C3 已落地；C1 已接上，真照片图配字仍等素材）
- C1：识字 `*-character-match` → `picture-match`（用现有字卡图，不新造图）；拼音新增 `pinyin-tone`「听音选调」
- C2：古诗/拼读 60 日已挂入；拼音无独立 60 日包，用 63 项题库 + 听音选调
- C3：家长互动页只读学情摘要
- C4：课时弹窗读 `fourSteps`/`evidence`，缺字段回退
