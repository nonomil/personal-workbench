...
| 来源 | 条款 | 关键句 |
|---|---|---|
| `docs/01-方案/工作台小游戏设计/02-方块世界/02-方块世界-落地改造方案.md` §6 任务1 | voxel 页文案改口 | 「把 voxel 脚本合同从 `jump\|crystal\|enemy\|steve` 改为必须出现 `quest\|inventory\|breakBlock\|placeBlock`，且 `index.html` 不再以『冲旗』为主提示」 |
| 同上 §7 版权 | 对外名称 | 「对外名称『方块世界』，不使用 Minecraft 商标」 |
| `docs/01-方案/工作台小游戏设计/03-横版闯关-落地改造方案.md` §7 版权 | 对外名称 | 「对外『横版闯关 / 彩虹闯关』，不出现 Mario、Luigi、问号砖作为可玩商标」 |
| `docs/01-方案/工作台小游戏设计/README.md` 第一性原理 §3 | 版权边界 | 「MIT 只管代码。PopCap / Nintendo / Mojang 贴图、关卡、音效不能进本仓」 |
| `docs/01-方案/徽章系统/DS--徽章系统设计.md` 问题6 | 真机验收 | 17 项验收清单（花园 3 / 地图 3 / 建造 3 / 三域 2 / UI 交互 6） |
| `docs/01-方案/优化方案2/幼小衔接工作台-方案文档/06-计划与验收/01-落地对照与缺口.md` | P0 原则 | 「主题切换无叠层」「做完一定有阳光且不重复刷」——本包不改这两条，仅作回归护栏 |
...
| B5 | 文档过时：笔顺 | `docs/01-方案/学习项目设计/01-落地分析-识字与字卡.md` §1 写「165 字笔顺按未实现处理」，实际 `prj/preschool-literacy-strokes-data.js` 已有 165 字（yxj-workbench / Make Me a Hanzi 兼容子集，247KB） |
