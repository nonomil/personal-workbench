...
- [ ] `docs/学习项目设计/2026-08-14-学习项目设计-落地分析.md` §1 表「165 字笔顺」行：裁决列由「按未实现处理」改为「**已实现**：`prj/preschool-literacy-strokes-data.js`（165 字，yxj-workbench 子集，Make Me a Hanzi 兼容）」；§10 本仓库现状行补该文件
- [ ] `docs/优化方案2/幼小衔接工作台-方案文档/03-伙伴与激励/02-徽章与券.md` 游戏券节首加裁决注记：
  > 裁决（2026-08-14）：本仓不实现券系统，趣味短局以学习阳光代替（`workbench-bridge.js` 日上限 80 + eventKey 去重）。依据 `docs/工作台小游戏设计/README.md`。下文保留为原始建议存档。
