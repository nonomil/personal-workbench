...
const userData = fs.mkdtempSync(path.join(os.tmpdir(), 'pw-garden-fire-'));
...
try {
  ...
  await cdp.evaluate(`(() => {
    ...
    state.growth.sunlight = Math.max(120, Number(state.growth.sunlight) || 0);
    bridge.writeState(state);
    const pea = [...document.querySelectorAll('.seed-card')].find((el) => (el.textContent || '').includes('豌豆'));
    if (pea) pea.click();
  })()`);
  ...
  await cdp.evaluate(`document.getElementById('spawn-btn').click()`);
  await sleep(2800);

  const snapshot = await cdp.evaluate(`(() => {
    const raw = localStorage.getItem(${JSON.stringify(KEY)});
    const state = raw ? JSON.parse(raw) : null;
    const defense = state && state.growth && state.growth.garden ? state.growth.garden.defense : {};
    return {
      noSkillBtn: !document.getElementById('skill-btn'),
      plantIds: (defense.plants || []).map((p) => p.plantId),
      zombieCount: (defense.zombies || []).length,
      zombieColumns: (defense.zombies || []).map((z) => z.column),
      peaCount: (defense.projectiles || []).length,
      status: defense.status,
      wave: defense.wave,
      tip: document.getElementById('message-tip')?.textContent || ''
    };
  })()`);

  if (!snapshot.noSkillBtn) throw new Error('skill button still present');
  if (!snapshot.plantIds.includes('plant-peashooter')) {
    throw new Error('peashooter not planted: ' + JSON.stringify(snapshot));
  }
  if (snapshot.zombieCount < 1) throw new Error('no zombies after spawn: ' + JSON.stringify(snapshot));
  const movedOrShot = snapshot.peaCount > 0 || snapshot.zombieColumns.some((col) => col < 5);
  if (!movedOrShot) throw new Error('peas did not fire and zombies did not walk: ' + JSON.stringify(snapshot));
