(function (global) {
        ...
        preschool: {
            ...
            childCourses: [
                ...
                },
                {
                    ...
                    lessons: [
                        { id: 'preschool-poetry-1', title: '朗读一首古诗', minutes: 10, meta: 'L1 · 从《静夜思》开始', tip: '听上句，选出下句。', activity: { mode: 'poetry-line', level: 'L1', preferred: 'poem-jingyesi', size: 5, prompt: '下一句是哪一句？', hint: '先听这一句。', options: ['听一句', '选下一句', '下一题'], answer: 0, optionIcons: ['moon', 'tree-pine', 'sparkles'], success: '诗句读懂啦！' } },
                        { id: 'preschool-poetry-2', title: '跟读一句', minutes: 8, meta: 'L1 · 听一句，学一句', tip: '从《咏鹅》接着听。', activity: { mode: 'poetry-line', level: 'L1', preferred: 'poem-yong-e', size: 5, prompt: '下一句是哪一句？', hint: '先听这一句。', options: ['听一句', '选下一句', '下一题'], answer: 0, optionIcons: ['moon', 'droplets', 'flower-2'], success: '画面想起来啦！' } },
                        { id: 'preschool-poetry-3', title: '记住诗意', minutes: 10, meta: 'L2 · 诗库连句', tip: '会听下一句也算过关。', activity: { mode: 'poetry-line', level: 'L2', preferred: 'poem-chunxiao', size: 5, prompt: '下一句是哪一句？', hint: '先听这一句。', options: ['听一句', '选下一句', '下一题'], answer: 0, optionIcons: ['book-marked', 'sprout', 'trees'], success: '诗意记住啦！' } }
                    ]
                },
                {
                    id: 'preschool-math',
    ...
    const labelMap = {
        ...
        '.sidebar-footnote': selected.id === 'preschool' ? 'v0.7.3 · 幼儿版' : selected.id === 'child' ? 'v0.7.3 · 儿童版' : 'v0.7.3 · 成人版',
    ...
    if (topbarActions && !topbarActions.querySelector('.topbar-workbench-switcher')) {
        ...
        ['adult', 'child'].forEach(function (id) {
            ...
            switchEntries.push({
                ...
                href: launcher && typeof launcher.getSiblingPath === 'function' ? launcher.getSiblingPath(selected.id, id) : (item.path || '../'),
                current: selected.id === id
            });
        });
        switchEntries.forEach(function (entry) {
            const link = document.createElement('a');
            link.className = `topbar-workbench-option ${entry.current ? 'is-current' : ''}`;
            link.href = entry.href;
            link.dataset.workbenchVariant = entry.variant;
            if (entry.theme) link.setAttribute('data-workbench-theme', entry.theme);
            if (entry.current) link.setAttribute('aria-current', 'page');
            link.innerHTML = `<i data-lucide="${entry.icon || 'layout-dashboard'}"></i><span><strong>${entry.name}</strong><small>${entry.summary || ''}</small></span>${entry.current ? '<b>当前</b>' : ''}`;
