...
test('preschool home surfaces three world progress and daily game sun cap', () => {
  ...
  assert.match(app.slice(homeStart, homeEnd), /renderPreschoolVoxelHomeCard\(\)/);
...
test('growth content tables expose multiple stages or quests', () => {
  ...
  assert.equal((levels.match(/\bL\(\d+/g) || []).length >= 16, true, 'platform should have 16 levels');
  assert.match(levels, /function plantOn\(/);
  assert.match(levels, /L\(11, '硬壳山谷'/);
  assert.match(levels, /L\(16, '终旗之夜'/);
  assert.equal((levels.match(/under: underMap\(/g) || []).length >= 8, true, 'mid and late levels should offer pipe rooms');
  assert.match(levels, /L\(1,\s*'青青草地',\s*6400/);
  assert.match(bridge, /ms-platform-speed/);
  assert.match(bridge, /闪电探险家/);
